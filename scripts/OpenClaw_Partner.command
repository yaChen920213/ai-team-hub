...
echo "🧠 Starting OpenClaw Partner (AI Team)..."
echo "Gateway on port 18800"

export OPENCLAW_STATE_DIR="$HOME/.openclaw-partner"
export OPENCLAW_CONFIG_PATH="$HOME/.openclaw-partner/openclaw.json"
...
if lsof -i :${OPENCLAW_GATEWAY_PORT} -sTCP:LISTEN -P >/dev/null 2>&1; then
  ...
  nohup openclaw gateway run > "$HOME/.openclaw-partner/logs/manual-gateway.log" 2>&1 &
  sleep 5
fi
